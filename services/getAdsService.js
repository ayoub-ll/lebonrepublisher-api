...
/*
(async () => {
    const {token, accountId} = await auth.getToken(process.env.lbc_username, process.env.lbc_password).then((result) => {
...
function request(token, accountId) {
  ...
  req.end(
    '{"AccountId":"' +
      accountId +
      '","Filters":{"Categories":[],"Keywords":"","State":null},"Format":"","Limit":100,"Page":1,"SortOrder":"Desc","SortParam":"LastToplistTime"}',
  )
